...
//ContentValues라는 값을 이용해서 안드로이드의 메소드를 이용
//SQL문이 실행될 수 있도록 작업
//메소드를 호출하며 적절한 값을 넘겨주면 안드로이드에서 SQL문을 만들어 실행
...
public class DBMainActivity_ContentValues extends AppCompatActivity {
    ...
    public void selectAll(View v){
        ...
        select * 컬럼명 from 테이블명 where 조건
          ...
          4. 조건으로 정의된 ?에 바인딩 될 값을 배열로 넘긴다.
          5. group by절 뒤에 정의할 컬럼명
          6. having에 정의할 조건
          7. order by 절에 정의할 정렬기준
    ...
    public void update(View v){
        ...
        db.update("memeber", contentValues, where, whereVal );
    }
